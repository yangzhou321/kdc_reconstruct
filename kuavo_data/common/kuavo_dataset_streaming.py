...
class StreamingRosbagProcessor:
    ...
    def scan_timestamps(self, bag_file: str) -> Tuple[str, List[float], Dict[str, List[float]]]:
        ...
        if self.sample_drop > 0:
            ...
            main_timestamps = main_timestamps_raw[::jump]
        
        logger.info(
            f"Generated {len(main_timestamps)} main timestamps "
            f"(from {len(main_timestamps_raw)} raw frames, "
            f"sample_drop={self.sample_drop}, jump={jump})"
        )
